...
{
    public partial class SelectUserFromAD : System.Web.UI.Page
    {
        ...
        public void IterateTreeView(TreeNodeCollection tnc)
        {
            foreach (TreeNode tn in tnc)
            {
                if (tn.Checked)
                {
                    ...
                    //{
                    string id = tn.Value;
                    if (id.StartsWith("LDAP"))
                    {
                        id = id.Replace("LDAP://","").Replace("OU=","");
                        string[] sz=id.Split(',');
                        id="";
...
//            DataTable dt2 = DataAccess.Instance("ResDB").ExecuteDataTable(sql);
            ...
            string sql = "select USERID as EmployeeID,USERNAME as FIRSTNAME,LOGINNAME as shortname,'' DepartmentName,'' Title from V_ORG_USER where USERNAME like '%"+searchText+"%'";
            DataTable dt2 = DataAccess.Instance("BizDB").ExecuteDataTable(sql);
            for (int i = 0; i < dt2.Rows.Count; i++)
            {
                DataRow dr = dt.NewRow();
                dr["EMPLOYEEID"] = domain + "/" + dt2.Rows[i]["EmployeeID"].ToString();
                dr["FIRSTNAME"] = dt2.Rows[i]["FIRSTNAME"].ToString();
                dr["SHORTNAME"] = domain + "/" + dt2.Rows[i]["SHORTNAME"].ToString();
                dr["DepartmentName"] = dt2.Rows[i]["DepartmentName"].ToString();
                dr["TITLE"] = "";
                dt.Rows.Add(dr);
            }
            return dt;
