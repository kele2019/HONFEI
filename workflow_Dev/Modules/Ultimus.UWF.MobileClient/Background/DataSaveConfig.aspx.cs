    ...
    public partial class DataSaveConfig : System.Web.UI.Page
    {
        ...
        private void BingData()
        {
            try
            {
                ...
                List<EntityLibrary.MobileClient_StepControl> List = StepControlDAL.GetModel("FK_ID='" + StepModel.ID + "' order by Convert(int,isnull(Orderby,0))");
            ...
            try
            {
                ...
                if (!String.IsNullOrEmpty(model.DestType))
                {
                    ...
                    if (model.DestType.IndexOf("ElectronicForm") >= 0)
                    {
                        ...
                        string ProcessName = Request.QueryString["ProcessName"].ToString().Trim();
                        string StepName = Request.QueryString["StepName"].ToString().Trim();
                        VariableName.DataSource = _ref.GetInitialVariable(ProcessName, StepName);//.GetVariableList(ProcessName, "S0421158c540122b61e67257aa2da7b");// UltimusEikOfIncidentDAL.GetVariable(ProcessName);
        ...
        protected void cbSaveToElectronicForm_CheckedChanged(object sender, EventArgs e)
        {
            ...
            Master.Enabled = Sublist.Enabled = true;
            _dropVariableName.Enabled = cb.Checked;
            string StepName = Request.QueryString["StepName"].ToString().Trim();
            _dropVariableName.DataSource = _ref.GetInitialVariable(Request.QueryString["ProcessName"].ToString().Trim(), StepName);
