	...
	public partial class NewRequest : System.Web.UI.Page
	{
		protected void Page_Load(object sender, EventArgs e)
		{
			if (!IsPostBack)
			{
				...
				{
					string FlagStatus = DataAccess.Instance("BizDB").ExecuteScalar("select  status from PROC_RemindSEP where INCIDENT='" + Incident + "' ").ToString();
		...
		protected void NewRequest_BeforeSubmit(object sender, CancelEventArgs g)
		{
            ...
            #endregion
            string sql = "select LOGINNAME from dbo.org_user where EXT03 <> 'GM' and EXT03 <> 'CTO' and ISACTIVE=1";
			List<EmployeeLoginEntity> lists = DataAccess.Instance("BizDB").ExecuteList<EmployeeLoginEntity>(sql);
			StringBuilder allEmployeeLogin = new StringBuilder();
			foreach(EmployeeLoginEntity employeeLogin in lists){
				allEmployeeLogin.Append(employeeLogin.LOGINNAME.Replace("\\","/") + "|USER,");
			}
			fld_ApprovalArr_AllEmployee.Text = allEmployeeLogin.ToString();
		}
