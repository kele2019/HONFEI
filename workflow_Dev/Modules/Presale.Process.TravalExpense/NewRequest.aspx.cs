    ...
    public partial class NewRequest : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
			{
                ...
                if (myRequest != null)
                {
                    if (myRequest.ToString() == "NEWREQUEST")
                    {
                        ...
                        userInfo.AddNewRow(fld_detail_Proc_TravalExpense_DT);
                    ...
                    if (Incident != null)
                    {
                        hdPrint.Value = DataAccess.Instance("BizDB").ExecuteScalar("select  COUNT(1) from PROC_TravalExpense where INCIDENT='" + Incident + "' and STATUS=2").ToString();
                    }
					if (Incident.ToString() != "0")
					{
						string FlagStatus = DataAccess.Instance("BizDB").ExecuteScalar("select  status from PROC_TravalExpense where INCIDENT='" + Incident + "' ").ToString();
        ...
        protected void NewRequest_BeforeSubmit(object sender, CancelEventArgs g)
        {
            ...
            DataAccess.Instance("BizDB").ExecuteNonQuery(strRevoke);
                ...
                //if (mode.LeaderName != mode.LeaderSuprName)//lineManager 和部门经理不相等
                //{
                //    table.Add("Level", 1);
                //}
                //else
    ...
    select  MEMBERID from ORG_GROUPMEMBER  where GROUPID=(select GROUPID from ORG_GROUP where   GROUPNAME='Fina Account'))";
        ...
        protected void NewRequest_AfterSubmit(object j, CancelEventArgs g)
        {
            try
            {
                ...
                DataAccess.Instance("BizDB").ExecuteNonQuery("delete from PROC_ResevserData where FormID='" + FormID + "' and RetrunType='TravelExpese'");
                if (ActiontType == 0)//提交
                {
                    ...
                    if (fld_BrrowYes.Checked)
                    {
                        foreach (var item in ReverseAmount.Split('|'))
                        {
                            ...
                            strsql.AppendFormat("insert into dbo.PROC_ResevserData (FormID,CANo,ReturnAmount,RetrunType,ReturnDate)  values('{0}','{1}','{2}','TravelExpese',GETDATE()) ", FormID, CashAdvanceNo, Amount);
        ...
        {
            UserInfo userInfo = Page.FindControl("UserInfo1") as UserInfo;
            userInfo.DeleteRow(fld_detail_Proc_TravalExpense_DT, e);
        }
